...
def render(
    ...
    with sync_playwright() as p:
        ...
        # For dark mode: do NOT use Excalidraw's exportWithDarkMode (it applies
        # a CSS filter that produces washed-out lavender, not true dark).
        # Instead, render normally and post-process the screenshot with PIL.
    ...
    # Post-process for dark mode: invert the image colors
    if dark:
        from PIL import Image, ImageOps
        img = Image.open(output_path).convert("RGB")
        img = ImageOps.invert(img)
        # Boost saturation slightly to compensate for inversion washing out colors
...
def main() -> None:
    ...
    parser.add_argument("--dark", "-d", action="store_true", help="Render with dark mode background")
    ...
    png_path = render(args.input, args.output, args.scale, args.width, args.dark)
    print(str(png_path))
